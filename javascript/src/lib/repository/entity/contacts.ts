...
export const contactsRepository = {
  bulkUpsertContact: async (contacts: ContactInput[]) => {
    ...
    const parsedContacts = contactSchemaInput.array().safeParse(contacts);

    if (parsedContacts.error) {
      logger.error(z.prettifyError(parsedContacts?.error), 'Contact validation error');
    }

    const contactsValid = parsedContacts.data;
    const db = getDbClient();

    const query = queryBuilder
      .insertInto('Contacts')
      .values(contactsValid.map((contact) => ({
        contact_id: contact.contactId,
        name: contact.name,
        email_address: contact.email === undefined ? null : contact.email,
        phone_number: contact.phoneNumber === undefined ? null : contact.phoneNumber,
        chat_app_username: contact.chatAppUsername === undefined ? null : contact.chatAppUsername,
    ...
    return contactSchema.array().parse(result.rows.map(row => ({
      contactId: row.contact_id,
      name: row.name,
      email: row.email_address === null ? undefined : row.email_address,
      phoneNumber: row.phone_number === null ? undefined : row.phone_number,
      chatAppUsername: row.chat_app_username === null ? undefined : row.chat_app_username,
  ...
  assignContactToChat: async (params: ContactChatInput)=> {
    ...
    }).onConflict(oc => oc.columns(['chat_id', 'contact_id']
    ).doUpdateSet({
        is_admin: params.isAdmin ? 1:0,
        // joined_at: <somethind>
      })
